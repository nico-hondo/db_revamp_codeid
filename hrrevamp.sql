...
CREATE TABLE hr.employee{
	...
	emp_gender char(1) CHECK (emp_marital_status in ('M', 'F')),
	emp_hire_date timestamp,
	emp_salaried_flag char(1) CHECK (emp_marital_status in ('0', '1')),
	emp_vacation_hours smallint,
	emp_sickleave_hours smallint,
	emp_current_flag smallint CHECK (emp_marital_status in (0, 1)),
	emp_modified_date timestamp,
	emp_type varchar(15) CHECK (emp_marital_status in ('Internal', 'Outsource')),
	emp_joro_id integer,
...
CREATE TABLE hr.employee_client_contract{
	...
	FOREIGN KEY (ecco_entity_id) REFERENCES hr.employee_client_contract(emp_entity_id),
